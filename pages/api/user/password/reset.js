...
handler.post(async (req, res) => {
  ...
  if (!user) {
    res.statusCode(401).send('The email is not found');
    return;
  }
  ...
  const msg = {
    ...
    html: `
      <div>
        <p>Hello, ${user.name}</p>
        <p>Please follow <a href="${process.env.WEB_URI}/forgot-password/${token}">this link</a> to reset your password.</p>
      </div>
    `
  };
...
handler.put(async (req, res) => {
  ...
  if (!tokenDoc) {
    res.status(403).send('This link may have been expires.');
    return;
  }
